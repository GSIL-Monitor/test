function viewPortIndex() {
	if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent)) {
		var a = $(window).width() < window.screen.width ? $(window).width() : window.screen.width,
			b = 320,
			c = a / b;
		a < b ? $("#Viewport").attr("content", "initial-scale=" + c + ", maximum-scale=" + c + ", minimum-scale=" + c + ", user-scalable=yes, width=" + a) : $("#Viewport").attr("content", "initial-scale=1.0, maximum-scale=2, minimum-scale=1.0, user-scalable=yes, width=" + a)
	}
}

function getid(a) {
	var b = new Image;
	b.src = "https://dws.ucloud.cn/report?type=event&tag=" + a
}

function getLoginStatus(a) {
	$.ajax({
		url: "//api.ucloud.cn/?Action=GetUserInfo",
		cache: !1,
		xhrFields: {
			withCredentials: !0
		},
...
function checkResource(a) {
	$.ajax({
		url: "//api.ucloud.cn/?Action=CheckResource",
		cache: !1,
		xhrFields: {
			withCredentials: !0
		},
...
function goRegisterPage() {
    location.href = window.isLogin ? "http://hdmes.com/UserAccount/UserReg" : "http://hdmes.com/UserAccount/UserRed"
}

function goLoginPage() {
    location.href = (isProduction ? "/UserAccount/UserLogin" : "UserAccount/UserLogin") + "?service=" + encodeURIComponent(location.href)
}

function Dropdown(a, b, c, d, e) {
	this.selectDom = a, this.dropdownDomWrapper = b, this.dropdownDoms = c, this.linkageDoms = d, this.callback = e, this.init()
}
...
function equalDiv(a) {
	...
	b.each(function() {
		...
		if (d && f <= d) return a.css("width", "");
		var g = e ? e : $(this).outerWidth(),
			h = a.length,
			i = parseInt(a.css("padding-left")) + parseInt(a.css("padding-right"));
		b.css("font-size", 0), a.width(Math.floor(g / h) - i)
	})
}
...
function switchTab(a, b) {
	var c = a.tab,
		...
		f = a.limit,
		g = viewport().width;
	return f && g <= f || "cancel" == b ? d.children(e).show() : void(hasHided(d.children(e)) || (c.children("li").first().addClass("active").siblings("li").removeClass("active"), d.children(e).first().show().siblings(e).hide(), c.find("a").click(function(a) {
		a.preventDefault();
		var b = $(this).attr("href");
		$(b).html() && ($(this).parent().addClass("active").siblings("li").removeClass("active"), $(b).fadeIn().siblings(e).hide())
	})))
}
...
function equalHeight(a) {
	var b = a.img,
		...
		f = a.limit,
		g = viewport().width;
	return f && g <= f ? void b.each(function() {
		var a = $(this).parents(d).find(c);
		return a.css("height", "")
	}) : void b.each(function() {
		var a, b = $(this),
			f = $(this).parents(d).find(c),
			g = f.height();
		if (g) {
			var h = new Image;
			h.src = b.attr("src"), h.onload = function() {
				a = h.height;
				var b = 800 * h.height / h.width,
					c = f.find(e);
				g < b ? c.removeClass("sticked") : (c.addClass("sticked"), f.height(b))
			}
		}
	})
}
...
function hasHided(a) {
	...
	return a.each(function() {
		if ("none" == $(this).css("display")) return b = !0
	}), b
}
...
function getImageSize(a, b) {
	function c() {
		f || (f = !0, clearInterval(e), a.off("load", d), b.apply(this, arguments))
	}
...
function createFlashPlayer(a) {
	var b = new ump.Player;
	b.create(a)
}

function anchorSwitch(a, b) {
	var c, d, e = a.$switch,
		f = a.eventMethod,
		g = a.isWatchUrl;
	if (e.children("li").on(f, function(a) {
			a.preventDefault();
			var e = $(this);
			e.addClass("active").siblings().removeClass("active"), c = e.children("a").attr("href"), d = $(c), d.length > 0 && b && b(d)
		}), g) {
		var h = /^#[0-9a-zA-Z\d]+$/;
		c = location.hash, h.test(c) && (d = $(c), d.length > 0 && ($("a[href=" + c + "]").parent().addClass("active").siblings().removeClass("active"), b && b(d)))
	}
}! function(a, b) {
	function c(a) {
	...
	function n(a, c, d, e, f, g) {
		...
		for (var h, i = a[f], j = 0, k = i ? i.length : 0, l = a === gb; j < k && (l || !h); j++) h = i[j](c, d, e), "string" == typeof h && (!l || g[h] ? h = b : (c.dataTypes.unshift(h), h = n(a, c, d, e, h, g)));
	...
	var H = a.document,
		...
		K = function() {
			...
			}), e = h.uaMatch(A), e.browser && (h.browser[e.browser] = !0, h.browser.version = e.version), h.browser.webkit && (h.browser.safari = !0), l.test("聽") && (m = /^[\s\xA0]+/, n = /[\s\xA0]+$/), d = h(H), H.addEventListener ? g = function() {
	...
	K.Callbacks = function(a) {
		...
		var c, d, e, f, g, h = [],
			...
			j = function(b) {
				var c, d, e, f;
				for (c = 0, d = b.length; c < d; c++) e = b[c], f = K.type(e), "array" === f ? j(e) : "function" === f && (!a.unique || !l.has(e)) && h.push(e)
			},
			k = function(b, j) {
				for (j = j || [], c = !a.memory || [b, j], d = !0, g = e || 0, e = 0, f = h.length; h && g < f; g++)
				...
				remove: function() {
					if (h)
						for (var b = arguments, c = 0, e = b.length; c < e; c++)
							for (var i = 0; i < h.length && (b[c] !== h[i] || (d && i <= f && (f--, i <= g && g--), h.splice(i--, 1), !a.unique)); i++);
					return this
				},
				has: function(a) {
					if (h)
	...
	K.extend({
		...
		when: function(a) {
			...
			var d = M.call(arguments, 0),
				...
				j = i.promise();
			if (f > 1) {
				for (; e < f; e++) d[e] && d[e].promise && K.isFunction(d[e].promise) ? d[e].promise().then(c(e), i.reject, b(e)) : --h;
				h || i.resolveWith(i, d)
		...
		data: function(a, c) {
			...
			if ("undefined" == typeof a) {
				if (this.length && (g = K.data(this[0]), 1 === this[0].nodeType && !K._data(this[0], "parsedAttrs"))) {
					e = this[0].attributes;
					for (var h = 0, i = e.length; h < i; h++) f = e[h].name, 0 === f.indexOf("data-") && (f = K.camelCase(f.substring(5)), F(this[0], f, g[f]));
	...
	K.fn.extend({
		...
		removeAttr: function(a, b) {
			var c, d, e, f, g = 0;
			if (b && 1 === a.nodeType)
				for (d = b.toLowerCase().split(T), f = d.length; g < f; g++) e = d[g], e && (c = K.propFix[e] || e, K.attr(a, e, ""), a.removeAttribute(Z ? e : c), Y.test(e) && c in a && (a[c] = !1))
		},
		attrHooks: {
			type: {
				set: function(a, b) {
	...
	K.fn.extend({
		show: function(a, b, c) {
			...
			for (var h = 0, i = this.length; h < i; h++) f = this[h], f.style && (g = f.style.display, !K._data(f, "olddisplay") && "none" === g && (g = f.style.display = ""), "" === g && "none" === K.css(f, "display") && K._data(f, "olddisplay", d(f.nodeName)));
			for (h = 0; h < i; h++) f = this[h], f.style && (g = f.style.display, "" !== g && "none" !== g || (f.style.display = K._data(f, "olddisplay") || ""));
			return this
		},
		hide: function(a, b, c) {
			if (a || 0 === a) return this.animate(e("hide", 3), a, b, c);
			for (var d, f, g = 0, h = this.length; g < h; g++) d = this[g], d.style && (f = K.css(d, "display"), "none" !== f && !K._data(d, "olddisplay") && K._data(d, "olddisplay", f));
	...
	var e = function(b, c) {
		this.options = a.extend({}, e.DEFAULTS, c), this.$element = b, this.$canvas = this._createCanvas(), this.startPoint = this._getCanvasHeight() * this.options.startPoint, this.xWidth = this._getRandomXwidth(this.options.minXwidth, this.options.maxXwidth), this.init()
	};
	e.DEFAULTS = {
		customWidth: 0,
		customHeight: 0,
		...
		_runDrawCurve: function(a, b, c, d, e, f) {
			for (var g, h, i, j, k, l, m, n, o = a.getContext("2d"), p = Math.ceil(this._getCanvasWidth() / this.xWidth), q = b > this.startPoint ? 2 * this.startPoint - b : 2 * (this.startPoint - b) + b, r = 0; r < this.options.curveNum; r++) {
				g = c[r], h = d[r], m = e[r], n = f[r];
				for (var s = 0; s < p + 1; s++) i = {
					x: this.xWidth * s - g,
					y: this.startPoint
				}, j = {
					x: this.xWidth * (s + 1) - g,
					...
					x: this.xWidth * (s + .5) - g,
					y: q
				}, o.save(), o.translate(0, h), o.strokeStyle = "rgba(255, 255, 255, " + m + ")", o.lineWidth = n, o.beginPath(), o.moveTo(i.x, i.y), o.bezierCurveTo(k.x, k.y, l.x, l.y, j.x, j.y), o.stroke(), o.restore()
			}
		},
		_clearCurve: function(a) {
			var b = a.getContext("2d");
			return b.clearRect(0, 0, this._getCanvasWidth(), this._getCanvasHeight()), this
		},
		_moveCurve: function(a, b, c) {
			function d() {
				h._clearCurve(a)._runDrawCurve(a, e, m, n, o, p), g && (e += 1, e >= f && (g = !1)), g || (e -= 1, e <= 0 && (g = !0)), setTimeout(d, b)
			}
			var e = 0,
				f = 2 * this.startPoint,
				g = !0,
				h = this;
			c && (e = c);
			for (var i, j, k, l, m = [], n = [], o = [], p = [], q = 0; q < this.options.curveNum; q++) i = this._getRandomXOffset(),
				m.push(i);
			for (var r = 0; r < this.options.curveNum; r++) j = this._getRandomSPOffset(), n.push(j);
			for (var s = 0; s < this.options.curveNum; s++) k = this._getRandomNum(this.options.minTransparent, this.options.maxTransparent), o.push(k);
			for (var t = 0; t < this.options.curveNum; t++) l = this._getRandomNum(this.options.minCurveWidth, this.options.maxCurveWidth), p.push(l);
			h._runDrawCurve(a, e, m, n, o, p), d()
		},
		_drawCurve: function() {
			var a, b = this.$canvas,
				c = b[0];
			return c.getContext ? (a = this._getRandomSpeed(), this._moveCurve(c, a)) : this._insertImg(), this
		},
		...
		_insertImg: function() {
			...
			return b.append(c), this
		},
		init: function() {
			this._insertCanvas(), this._drawCurve()
		}
	}, a.fn.drawBezierCurve = d, a.fn.drawBezierCurve.Constructor = e
	...
	var e = function(b, c) {
		this.options = a.extend({}, e.DEFAULTS, c), this.$element = b, this.$canvas = this._createCanvas(), this.amplitude = this._getRandomA(this.options.minAmplitude, this.options.maxAmplitude), this.omega = this._getRandomOmega(this.options.minOmega, this.options.maxOmega), this.init()
	};
	e.DEFAULTS = {
		minAmplitude: 50,
		maxAmplitude: 100,
		...
		_drawCurve: function() {
			var a = this.$canvas,
				b = a[0];
			return b.getContext && this._runDrawCurve(b), this
		},
		...
		_runDrawCurve: function(a) {
			var b = a.getContext("2d");
			b.lineWidth = 3, b.strokeStyle = "rgba(255, 255, 255,.5)";
			var c = 0,
				d = this._sinFun(c),
				e = this,
				f = setInterval(function() {
					c <= e._getWrapWidth() ? (b.beginPath(), b.moveTo(c, d), c += 1, d = e._sinFun(c), b.lineTo(c, d), b.stroke()) : clearInterval(f)
				}, this.options.speed);
			return this
		},
		_insertCanvas: function() {
	...
	}, a.fn.drawCurve = d, a.fn.drawCurve.Constructor = e
...
var FlashDetect = new function() {
	...
	a.majorAtLeast = function(b) {
		...
		if (navigator.plugins && navigator.plugins.length > 0) {
			...
			for (var h = -1, j = 0; j < b.length && h == -1; j++) {
				var k = d(b[j].name);
				if (!k.activeXError && (a.installed = !0, h = b[j].version(k), h != -1)) {
					var i = e(h);
...
FlashDetect.JS_RELEASE = "1.0.4", + function(a, b, c) {
		function d(b) {
			return this.each(function() {
				var c = a(this);
				new e(c, b)
		...
		var e = function(b, c) {
			this.options = a.extend({}, e.DEFAULTS, c), this.$element = b, this.$bannerList = b.children(this.options.bannerListWrap).children("li"), this.$dotsWrap = b.find(".dots-wrap-index"), this.$carousel = b.find(this.options.carousel), this.bannerNum = this.$bannerList.length, this.init()
		};
		e.DEFAULTS = {
			bannerListWrap: ".banner-list-index",
			...
			_getCurrentBanner: function() {
				var b;
				return this.$bannerList.each(function(c) {
					a(this).hasClass("active") && (b = c)
				}), b
			},
			_insertDots: function() {
				var b, c = this.$dotsWrap,
					d = a('<ul class="dots"></ul>'),
					e = '<li class="dot"></li>';
				for (b = 1; b < this.bannerNum; b++) e += '<li class="dot"></li>';
				return a(e).appendTo(d), d.appendTo(c), this
			},
			_setDotsActive: function(a) {
				var b;
				return b = isNaN(a) ? this._getCurrentBanner() : a, this.$dotsWrap.find(".dot").eq(b).addClass("active").siblings(".dot").removeClass("active"), this
			},
			_setActiveBgColor: function(a) {
				var b;
				b = isNaN(a) ? this._getCurrentBanner() : a;
				var c = this.$bannerList.eq(b).data("bgcolor");
				return this.$element.css("background-color", c), this
			},
			_showBanner: function(b, c) {
				var d = this.$bannerList;
				if (!d.is(":animated")) return "pre" == c ? d.eq(b).addClass("active").show() : (d.eq(b).siblings().removeClass("active"), d.eq(b).addClass("active").fadeIn(400, function() {
					a(this).siblings().hide()
				})), this._setDotsActive(b), this._setActiveBgColor(b), this
			},
			_changeBanner: function(a) {
				var b, c = this._getCurrentBanner(),
					d = this.bannerNum;
				return "plus" == a && (b = c + 1 == d ? 0 : c + 1), "minus" == a && (b = c - 1 == -1 ? d - 1 : c - 1), this._showBanner(b), this
			},
			_runShowBanner: function(a) {
				var b = this;
				return "go" == a && (this.interval = setInterval(function() {
					return b._changeBanner("plus")
				}, b.options.speed)), "stop" == a && clearInterval(this.interval), this
			},
			_dotClickChange: function() {
				var b, c = this;
				return this.$dotsWrap.find(".dot").on("click", function() {
					b = a(this).index(), c._showBanner(b), c._runShowBanner("stop")._runShowBanner("go")
				}), this
			},
			...
			_carouselClickChange: function() {
				var b, c = this.$carousel,
					d = this;
				return c.on("click", function() {
					b = a(this).hasClass("left") ? "left" : "right", d._runDirChange(b)
				}), this._getTouchDir(function(a) {
					d._runDirChange(a)
				}), this
			},
			_runDirChange: function(a) {
				if (a) return "left" == a ? (this._changeBanner("minus"), this._runShowBanner("stop")._runShowBanner("go")) : (this._changeBanner("plus"), this._runShowBanner("stop")._runShowBanner("go")), this
			},
			_pauseShowBanner: function() {
				var a = this;
				return this.$bannerList.hover(function() {
					a._runShowBanner("stop")
				}, function() {
					a._runShowBanner("go")
				}), this
			},
			init: function() {
				this.options.dots && this._insertDots(), this._showBanner(0, "pre"), this._dotClickChange(), this._carouselClickChange(), this._runShowBanner("go"), this._pauseShowBanner()
			}
		}, a.fn.moveBanner = d, a.fn.moveBanner.Constructor = e
	}(jQuery, window, document), + function(a, b, c) {
		function d(b) {
			return this.each(function() {
				var c = a(this);
		...
		var e = function(b, c) {
			this.options = a.extend({}, e.DEFAULTS, c), this.$element = b, this.subEleNum = this._getSubjects().length, this.init()
		};
		e.DEFAULTS = {
			subMarginSize: "12px",
			...
			_disableLinks: function(b) {
				b && b.on("click", function(b) {
					a(this).is("[disabled]") && b.preventDefault()
				}), this._getSubjects().on("click", function(b) {
					a(this).is("[disabled]") && b.preventDefault()
				})
			},
			_setActiveLinkToWrap: function(a) {
				var b, c = a ? a : this._getSubjects("active"),
					d = c.find(".solu-btn");
			...
			_setEleWidth: function() {
				return this.$element.find(this.options.subWrapName).css({
					width: this._getEleWidth() + 10
				})
			},
			_getCenter: function() {
			...
			_moveToCenter: function(c, d) {
				var e, f, g, h, i = this.$element.find(this.options.subWrapName),
					j = isNaN(d) ? 500 : d,
					k = this;
				if (!i.is(":animated")) return i.animate({
					marginLeft: "-" + c + "px"
				}, {
					duration: j,
					complete: function() {
						e = k._getSubjects().first(), f = k._getSubjects().last(), g = e.offset().left, h = a(b).width() - f.offset().left - k._getSubWidth();
						var c = Math.ceil((g + k._getMvDistance()) / k._getMvDistance()),
							d = Math.ceil((h + k._getMvDistance()) / k._getMvDistance());
						if (c > 0) {
							for (var j = 0; j < c; j++) f = k._getSubjects().last(), f.remove().prependTo(k.options.subWrapName), k._disableLinks(f);
							i.css("margin-left", "-=" + k._getMvDistance() * c), k._setSubOpacity(k._getSubjects("active"))
						}
						if (d > 0) {
							for (var l = 0; l < d; l++) e = k._getSubjects().first(), e.remove().appendTo(k.options.subWrapName), k._disableLinks(e);
			...
			_runMainLayout: function() {
				this._disableLinks();
				var a = this._getSubjects("active"),
					b = a.index();
			...
			_aniSwitch: function(a, b) {
				return b ? a.show().animate({
					opacity: 1
				}, 500).siblings(b).attr("style", "") : a.css({
					opacity: 0
				}).animate({
					opacity: 1
				}, 500)
			...
			_showCurrentActiveList: function() {
				var b = this._getSubjects("", this.options.subListName),
					c = a(this.options.subPointer),
					d = this._getCurrentActiveSub().index();
			...
			_controlToCenter: function() {
				var b, c = this,
					d = this.$element.find(this.options.moveControl);
				this._getTouchDir(function(a) {
					var b;
					b = "left" != a, c._runSubMove(b)
				}), d.on("click", function() {
					b = a(this).hasClass("left"), c._runSubMove(b)
				})
			},
			_getToCentreDistance: function(b) {
				var c, d, e, f = this._getSubjects(),
					g = this;
				f.on("click", function(f) {
					c = g._getSubjects("active").index(), d = a(this).index(), e = d - c, b && b(d, e)
				})
			},
			_ctrlClickToCentre: function() {
				var a, b = this;
				return this._getToCentreDistance(function(c, d) {
					a = b._getSubjects().eq(c), d > 0 ? b._runSubMove(!1, Math.abs(d), c) : d < 0 && b._runSubMove(!0, Math.abs(d), c)
				}), this
			},
			...
			_handleDetailBtn: function() {
				var b = this._getSubjects(),
					c = b.find("a"),
					d = this;
				c.click(function(b) {
					var c = a(this).parents(d.subName).attr("disabled");
					"disabled" != c && b.stopPropagation()
				})
			},
			init: function() {
				var c = this;
				this._disableLinks(), this._runMainLayout(), this._setActiveLinkToWrap(), a(b).resize(function() {
					c._reFreshLayout()
				}), this._controlToCenter(), this._ctrlClickToCentre(), this._handleDetailBtn()
			}
		}, a.fn.moveFilm = d, a.fn.moveFilm.Constructor = e
	}(jQuery, window, document), + function(a, b, c) {
		function d(b) {
			return this.each(function() {
				var c = a(this);
		...
		var e = function(a, b) {
			this.$element = a, this.$wrapLevel01 = this._getLevelWrapper("01"), this.$wrapLevel02 = this._getLevelWrapper("02"), this.$wrapLevel03 = this._getLevelWrapper("03"), this.init()
		};
		e.prototype = {
			_getMainWrapper: function() {
				return this.$element.children(".menu")
			},
			_getLevelWrapper: function(a) {
				return this._getMainWrapper().children(".menu-level" + a)
			},
			_getMenuItem: function() {
				return this.$element.find(".sub-menu:not(.black)")
			},
			_getBackItem: function() {
				return this.$element.find(".sub-menu.black")
			},
			_changeMenu: function() {
				var b = this._getMenuItem(),
					c = this;
				return b.on("click", function(b) {
					c._runChangeMenu(a(this)), b.stopPropagation()
				}), this
			},
			_returnBack: function() {
				var b = this._getBackItem(),
					c = this;
				return b.on("click", function(b) {
					c._runReturnBack(a(this)), b.stopPropagation()
				}), this
			},
			_runChangeMenu: function(a) {
				var b = a.parents(".menu-wrapper"),
					c = 0 == a.index() ? 0 : a.index() / 2;
				if (b.hasClass("menu-level01") && (this.$wrapLevel01.hide(), this.$wrapLevel02.show().children(".sub-menu-wrapper").eq(c).show()), b.hasClass("menu-level02")) {
					var d = a.parent().index();
					c = 0 == a.index() ? 0 : (a.index() - 1) / 2, this.$wrapLevel02.hide().find(".sub-menu-wrapper").hide(), this.$wrapLevel03.show().children(".sub-menu-wrapper").eq(d).show().children(".sub-menu-wrapper").eq(c).show()
				}
				if (!b.hasClass("menu-level03")) return this
			},
			_runReturnBack: function(a) {
				var b = a.parents(".menu-wrapper");
				if (b.hasClass("menu-level02") && (this.$wrapLevel02.hide().find(".sub-menu-wrapper").hide(), this.$wrapLevel01.show()), b.hasClass("menu-level03")) {
					var c = a.parent().parent().index();
					this.$wrapLevel03.hide().find(".sub-menu-wrapper").hide(), this.$wrapLevel02.show().children(".sub-menu-wrapper").eq(c).show()
				}
				return this
			},
			_linkToToggle: function() {
				var b, c = this._getMenuItem(),
					d = this;
				return c.on("click", function() {
					b = a(this).attr("href"), b && (a(".mobile-wrapper").toggleClass("nav-open"), d._getMainWrapper().toggle())
				}), this
			},
			init: function() {
				var b = this;
				this.$element.on("click", function() {
					a(".mobile-wrapper").toggleClass("nav-open"), b._getMainWrapper().toggle(), b.$wrapLevel01.show(), b.$wrapLevel02.hide().find(".sub-menu-wrapper").hide(), b.$wrapLevel03.hide().find(".sub-menu-wrapper").hide()
				}), this._changeMenu(), this._returnBack(), this._linkToToggle()
			}
		}, a.fn.multiMenu = d, a.fn.multiMenu.Constructor = e
	}(jQuery, window, document), + function(a, b, c) {
		function d(b) {
			return this.each(function() {
				var c = a(this);
		...
		var e = function(b, c) {
			this.options = a.extend({}, e.DEFAULTS, c), this.$element = b, this.init()
		};
		e.DEFAULTS = {
			startClass: "animated"
		}, e.prototype = {
			_isElementInViewport: function(c) {
				var d = navigator.userAgent.toLowerCase().indexOf("webkit") != -1 ? "body" : "html",
					e = a(d).scrollTop(),
					f = e + a(b).height(),
					g = Math.round(c.offset().top),
					h = g + c.height();
				return g < f && h > e
			},
			_checkAnimation: function() {
				var a = this.$element,
					b = this.options.startClass;
				if (!a.hasClass(b)) return this._isElementInViewport(a) && a.addClass(b), this
			},
			init: function() {
				var c = this;
				a(b).scroll(function() {
		...
		}, a.fn.scrollAni = d, a.fn.scrollAni.Constructor = e
	}(jQuery, window, document), + function(a, b, c) {
		function d(b) {
			return this.each(function() {
				var c = a(this);
		...
		e.DEFAULTS = {
			...
			_getMainEle: function() {
				return this.$element.children(this.options.wrapName)
			},
			_getSingleEle: function() {
				var a = this._getMainEle();
				return a.children("li")
			},
			_getSingleWidth: function() {
				return this._getSingleEle().outerWidth()
			},
			_copyEle: function() {
				var c = a(b).width(),
					d = this._getMainEle().outerWidth(),
					e = Math.ceil(c / d),
					f = this._getSingleEle();
				if (e >= 1)
					for (var g = 0; g < e; g++) this._getMainEle().append(f.clone());
				return this
			},
			_moveSingle: function() {
				var a = this._getMainEle(),
					b = this._getSingleWidth(),
					c = this;
				return a.is(":animated") || a.animate({
					marginLeft: "-=" + b + "px"
				}, {
					duration: this.options.singleSpeed,
					easing: "swing",
					complete: function() {
						c._changeEle(), a.css("margin-left", 0)
					}
				}), this
			},
			_changeEle: function() {
				var a = this._getMainEle(),
					b = this._getSingleEle().first();
				return b.remove().appendTo(a), this
			},
			_moveMulti: function() {
				var a = this._getMainEle(),
					b = this._getSingleWidth(),
				...
				return a.animate({
					...
					duration: this.options.multiSpeed,
					easing: "linear",
					complete: function() {
						c._changeEle(), a.css("margin-left", 0), c._moveMulti()
					}
				}), this
			},
			_runMove: function() {
				function a() {
					c._moveSingle(), setTimeout(a, c.options.duration)
				}
				var b = this.options.moveType,
					c = this;
				return "single" == b ? a() : this._moveMulti(), this
			},
			init: function() {
				var a = this;
				this._copyEle(), "single" == this.options.moveType ? setTimeout(function() {
					a._runMove()
				}, this.options.firstDuration) : a._runMove()
			}
		}, a.fn.slideShow = d, a.fn.slideShow.Constructor = e
	}(jQuery, window, document),
	function(a) {
				...
				function b(b) {
					if (g) {
						var d = b.touches[0].pageX,
							...
							i = e - d,
							j = f - h;
						Math.abs(i) >= c.min_move_x ? (c.scrollX || b.preventDefault(), a(), i > 0 ? c.wipeLeft() : c.wipeRight()) : Math.abs(j) >= c.min_move_y && (c.scrollY || b.preventDefault(), a(), j > 0 ? c.wipeDown() : c.wipeUp())
					}
				}
		...
		var b = window.location.href,
			c = !1,
			d = c ? "https://st.ucloud.cn/" : "https://www.ucloud.cn/";
		"http://www.eva.frontend.ucloud.cn/index01.html" == b || b == d || b == d + "site/" || b == d + "en/" || b == d + "site/index.html" || b == d + "en/index.html" ? $(".top-bg").addClass("index") : $(".top-bg").removeClass("index"), b == d || b == d + "site/product/*" ? viewPortIndex() : $("#Viewport").remove()
		...
		function changeUrlPar(url, par, value) {
			var pattern = par + "=([^&]*)",
				replaceText = par + "=" + value;
			if (url.match(pattern)) {
				var tmp = "/\\" + par + "=[^&]*/";
				return url = url.replace(eval(tmp), replaceText)
			}
			return url.match("[?]") ? url + "&" + replaceText : url + "?" + replaceText
		}

		function showBanner(a) {
			banner_list_img.eq(a).addClass("active").fadeIn(750).siblings("li").fadeOut(750).removeClass("active");
			var b = banner_list_img.eq(a).data("bgcolor");
			$(".banner01").css("background-color", b)
		}

		function changeBanner() {
			showBanner(show_index), show_index++, show_index == banner_list_img.length && (show_index = 0), dot.eq(show_index - 1).addClass("active").siblings(".dot").removeClass("active")
		}

		function uepForm() {
			function a(a) {
				a.val() || a.siblings(".s-email-form-error").show()
			}
			var b = $("#uep-name"),
				c = $("#uep-connect"),
				d = $("#uep-code"),
				...
				i = e.val();
			a(b), a(c), a(d), a(e);
			var j = "name=" + f + "&connect=" + g + "&code=" + h + "&info=" + i + "&action=post&diyid=1&do=2&dede_fields=name,varchar;connect,varchar;code,varchar;info,textchar";
			f && g && h && i && $.ajax({
				type: "POST",
				url: "/plus/diy.php",
				data: j,
		...
		function supportUrl(a) {
			checkResource(function(a) {
				var b, c = a.RetCode,
					d = a.IsExist,
					e = $(".J_go-support"),
					f = $(".J_modal-title"),
					g = $(".J_modal-content");
				switch (e.unbind("click"), $(".modal-laywer").fadeIn(), c) {
					case 0:
						return e.click(function() {
							"Yes" == d ? window.open(support_url_after) : window.open(support_url_before)
						}), void("Yes" == d ? (b = '<span class="icon__circle-i support-modal-icon"></span> <span class="confirm-content">楠岃瘉鎴愬姛锛岀偣鍑绘寜閽紑濮嬪湪绾縌Q鍜ㄨ銆�</span>', f.text("楠岃瘉鎴愬姛"), g.html(b), $(".uc-btn.J_close-modal").hide(), e.text("绔嬪嵆鍜ㄨ")) : ($(".uc-btn.J_close-modal").show(), b = '<span class="icon__circle-i support-modal-icon"></span> <span class="confirm-content">楠岃瘉澶辫触锛屼綘杩樻湭璐拱浠讳綍璧勬簮銆傛槸鍚﹁繘琛屽敭鍓嶅挩璇紵</span>', f.text("楠岃瘉澶辫触"), g.html(b), e.text("鍞墠鍜ㄨ")));
					case 170:
						return void goLoginPage();
					default:
						e.click(function() {
							window.open(support_url_before)
						}), $(".uc-btn.J_close-modal").show(), b = '<span class="icon__circle-mark"></span> <span class="confirm-content">楠岃瘉澶辫触,鏄惁杩涜鍞墠鍜ㄨ锛�</span>', f.text("楠岃瘉澶辫触"), g.html(b), e.text("鍞墠鍜ㄨ")
				}
			})
		}
		var Ytag = getQueryString("ytag");
		null != Ytag && (document.cookie = "ytag=" + encodeURIComponent(Ytag) + ";domain = .ucloud.cn;Path=/"), $(".hot-search li").click(function() {
			window.location.href = changeUrlPar(window.location.href, "question", $(this).html())
		});
		var $subMenu = $(".sub-menu"),
			$menuDropdownClose, $menuDropdown, menuAutoHeight;
		$subMenu.hover(function() {
			return $menuDropdown = $("#" + $(this).data("href")), $menuDropdown.height() > 0 ? void $menuDropdown.stop(!0, !0) : ($menuDropdownClose = $(".menu-dropdown:not(#" + $(this).data("href") + ")"), $menuDropdownClose.stop(!0, !0).delay(100).queue(function(a) {
				$(this).removeClass("open"), a()
			}).animate({
				height: 0,
				"z-index": 0
			}, 1), $(".menu").find(".menu-dropdown").hasClass("open") && (menuAutoHeight = $menuDropdown.css("height", "auto").height(), $menuDropdown.css({
				height: 0,
				opacity: 0
			}).delay(200).animate({
				"z-index": 1,
			...
			})), menuAutoHeight = $menuDropdown.css("height", "auto").height(), void $menuDropdown.css({
				height: 0,
				opacity: 0
			}).animate({
				"z-index": 1,
			...
			$menuDropdown = $("#" + $(this).data("href")), $menuDropdown.stop(!0, !0).delay(100).queue(function(a) {
				$(this).removeClass("open"), a()
			}).animate({
				height: 0,
				"z-index": 0
		...
		var $sidebarLink = $(".sidebar-link"),
			sidebarData, $sidebarContent, $navParentDrop, currNavHeight, navParentHeight;
		$sidebarLink.click(function(a) {
			a.preventDefault()
		}), $sidebarLink.mouseenter(function() {
			sidebarData = $(this).data("href"), $sidebarContent = $("#" + sidebarData), $(this).addClass("hover").siblings().removeClass("hover"), $sidebarContent.addClass("show").siblings().removeClass("show"), $navParentDrop = $(this).parents(".menu-dropdown"), currNavHeight = $navParentDrop.height(), navParentHeight = $navParentDrop.css("height", "auto").height(), $navParentDrop.css("height", currNavHeight).stop(!0, !0).animate({
				height: navParentHeight
			}, 200)
		});
		var $menuPdList, $menuSubWrapper = $(".menu-sub-wrapper"),
			$menuPdItem;
		$menuSubWrapper.each(function() {
			$menuPdItem = $(this).children(".menu-pd-item");
			var a = "",
				b = "",
				c = 0;
			if ($menuPdItem.length <= 4) {
				for (c = 0; c < $menuPdItem.length; c++) a += $menuPdItem.eq(c)[0].outerHTML, $menuPdItem.eq(c).remove();
				$menuPdList = $('<div class="menu-pd-list">' + a + "</div>"), $menuPdList.appendTo(this)
			} else {
				for (c = 0; c < $menuPdItem.length; c++) c <= 3 ? (a += $menuPdItem.eq(c)[0].outerHTML, $menuPdItem.eq(c).remove()) : (b += $menuPdItem.eq(c)[0].outerHTML, $menuPdItem.eq(c).remove());
				$menuPdList = $('<div class="menu-pd-list">' + a + '</div><div class="menu-pd-list">' + b + "</div>"), $menuPdList.appendTo(this)
			}
		});
		var $service_tool_link = $(".J_service-tool-link"),
			$service_tool_tips = $(".service-tool-tips"),
			service_timer;
		$service_tool_link.click(function(a) {
			a.preventDefault()
		}), $service_tool_link.mouseenter(function() {
			var a = $(this).attr("href");
			$service_tool_tips.each(function() {
				$(this).removeClass("fade-in")
			}), $(a).addClass("fade-in")
		}), $service_tool_link.mouseleave(function() {
			var a = $(this).attr("href");
			service_timer && clearTimeout(service_timer), service_timer = setTimeout(function() {
				$(a).removeClass("fade-in")
			}, 200)
		}), $service_tool_tips.each(function() {
			$(this).hover(function() {
				$(this).hasClass("fade-in") && service_timer && clearTimeout(service_timer)
			}, function() {
				$(this).removeClass("fade-in")
			})
			...
			$(this).removeClass("db")
		}), $("ul.tabs-nav li,.anchor li,.about-gsjj li").click(function() {
			$(this).addClass("active").siblings().removeClass("active")
		}), $(".anchor li,.about-gsjj li").click(function(a) {
		...
		var tabs = $(".tabs-nav li"),
			tab_contents = $(".tab-content .tab-pane");
		tabs.find("a").click(function(a) {
			a.preventDefault();
			var b = $(a.currentTarget),
				c = b.attr("href");
			tabs.removeClass("active"), b.parents(".tab").addClass("active"), tab_contents.removeClass("active"), $(c).addClass("active")
		}), $("#mobile-lock").click(function() {
			$(this).toggleClass("active")
		}), $(".tab-content .title").click(function() {
			var a = $(this).find(".caret-blue");
			a.hasClass("top") ? a.removeClass("top").addClass("bottom") : a.removeClass("bottom").addClass("top"), $(this).next(".tab-pane").toggleClass("active")
		}), $(".product-sort .sort-name").click(function(a) {
			$(".product-sort .product-list").removeClass("active"), $(".sort-name .caret-white").removeClass("top").addClass("bottom");
			var b = $(this).find(".caret-white");
			b.hasClass("top") ? (b.removeClass("top").addClass("bottom"), $(this).next(".product-list").removeClass("active")) : (b.removeClass("bottom").addClass("top"), $(this).next(".product-list").addClass("active"))
		}), $(".main-nav .first-node.father").click(function() {
			$(this).toggleClass("active");
			var a = $(this).siblings(".active");
			a.removeClass("active")
		}), $(".main-nav .first-node .nav>li").click(function(a) {
			a.stopPropagation()
			...
			var a = $(".solution"),
				...
				c = $(".about-gsjj"),
				d = $(".usafe"),
				e = $(".sub-tabs-x.tic-2017");
			$(document).scrollLeft(), $(window).width();
			$(document).scrollTop() > 441 ? d.addClass("usec-fixed") : d.removeClass("usec-fixed"), $(document).scrollTop() > 441 ? a.addClass("solu-fixed") : a.removeClass("solu-fixed"), $(document).scrollTop() > 470 ? b.addClass("fixed") : b.removeClass("fixed"), $(document).scrollTop() > 234 ? c.addClass("fixed") : c.removeClass("fixed"), $(document).scrollTop() > 993 ? e.addClass("fixed") : e.removeClass("fixed")
		}), $(".link-wrap.back-to-top").click(function() {
		...
		scrollTop >= winHeight ? $back_to_top.show() : $back_to_top.hide(), $service_tool.css("margin-bottom", -$service_tool.height() / 2 + "px"), $(window).scroll(function() {
			var a = $(document).scrollTop(),
				b = $(window).height();
			$(document).height() - a - b, $(".service-tool"), $(".page-footer-wrap").height();
			a >= b ? $back_to_top.is(":animated") || $back_to_top.fadeIn(100) : $back_to_top.is(":animated") || $back_to_top.fadeOut(100)
		});
		var join_tabs = $("#my-tab .tab"),
			join_tab_contents = $("#my-tab-content .tab-pane");
		join_tabs.find("a").click(function(a) {
			a.preventDefault();
			var b = $(a.currentTarget),
				c = b.attr("href");
			join_tabs.removeClass("active"), b.parents(".tab").addClass("active"), join_tab_contents.removeClass("active"), $(c).addClass("active")
		});
		...
		dot.bind("click", function() {
			var a = $(this).index();
			$(this).addClass("active").siblings(".dot").removeClass("active"), show_index = a, showBanner(a)
		}), changeBanner();
		var autoChange = setInterval(changeBanner, 6e3);
		banner_list.hover(function() {
			clearInterval(autoChange)
		}, function() {
			autoChange = setInterval(changeBanner, 6e3)
		});
		var page = 1,
			i = 2;
		$("a .next").click(function() {
			var a = $(".content"),
				...
				d = a.find("li").length,
				e = Math.ceil(d / i);
			b.is(":animated") || (page == e ? (b.animate({
				left: "0px"
			}, "slow"), page = 1) : (b.animate({
				left: "-=" + c
			}, "slow"), page++))
			...
			var a = $(".content"),
				...
				d = a.find("li").length,
				e = Math.ceil(d / i);
			b.is(":animated") || (1 == page ? (b.animate({
				left: "-=" + c * (e - 1)
			}, "slow"), page = e) : (b.animate({
				left: "+=" + c
			}, "slow"), page--))
		}), getLoginStatus(function(a) {
			0 == a.RetCode && ($(".logined").addClass("dib").removeClass("dn").find(".user-name").text(a.DataSet[0].UserEmail), window.isLogin = !0, $(".jsRegisterBtn").text("杩涘叆鎺у埗鍙�"), $(".nologin").addClass("dn"))
		}), $(".nologin .login").click(function() {
			goLoginPage()
		}), $(".bannerbg").moveBanner(), $(".solution-wear").moveFilm(), $("#curve-pd").drawBezierCurve({
			minXwidth: 850,
			maxXwidth: 1e3,
			startPoint: .2,
			minMoveSpeed: 80,
			...
			minTransparent: .2,
			maxTransparent: .6
		}), $(".mob-nav").multiMenu(), $("#film-show").slideShow({
			moveType: "multi",
			multiSpeed: 15e3
		}), $(".text-wrap").scrollAni({
			startClass: "animated"
		}), $(".landing-link-wrap").scrollAni({
			startClass: "animated"
		}), $(".sup-wrap").scrollAni({
			startClass: "ani-start"
		}), $(".landing-bottom-sup").scrollAni({
			startClass: "animated"
		}), $(".J_uep-banner").moveBanner({
			bannerListWrap: ".banner-main"
		}), $("#uep-nav").click(function() {
			$(".navbar-drawer-bar").toggleClass("translate-right"), $(".strikingly-drawer").toggleClass("translate")
		}), $("#uep-top-nav a").click(function() {
			$(this).parents().find("a").removeClass("selected"), $(this).addClass("selected")
		}), $("#uep-submit").click(function(a) {
			a.preventDefault(), uepForm()
		}), equalHeight({
			img: $(".wrapper-img img"),
			content: ".wrapper-text",
			wrapper: ".solution-structure-content",
			btn: ".solution-context-btn",
			limit: 1024
		}), equalHeight({
			img: $(".wrapper-img img"),
			content: ".wrapper-text",
			wrapper: ".content-with-img",
			btn: ".solution-context-btn",
			limit: 1024
		}), equalDiv({
			parent: $(".solution-advantages-wrapper"),
			child: ".advantage-cell",
			limit: 1024
		}), equalDiv({
			parent: $(".solution-switch-tab"),
			child: "li",
			limit: 1024
		}), equalDiv({
			parent: $(".solution-cases"),
			child: "li"
		}), equalDiv({
			parent: $(".solution-content-section>.list-wrapper"),
			child: ".list-cell",
			limit: 1024
			...
			var a = {
				tab: $(".solution-switch-tab"),
				content: $(".solution-structure"),
				selector: ".solution-structure-content"
			};
			viewport().width <= 1024 ? switchTab(a, "cancel") : switchTab(a)
		}), switchTab({
			tab: $(".solution-switch-tab"),
			content: $(".solution-structure"),
			selector: ".solution-structure-content",
		...
		if ("undefined" != typeof videojs)
			if (isChrome) {
				if ($("#solution-video-flash").remove(), $("#solution-video-h5").length > 0) var vjsSolution = videojs("#solution-video-h5", {
					width: 800,
					height: 450,
					poster: "",
					controls: !0,
					...
					this.on("loadstart", function() {
						c > 5 ? (c = 0, b = 0) : a.currentTime(b)
					}), this.on("loadeddata", function() {}), this.on("seeking", function() {}), this.on("error", function() {
						b = a.currentTime(), c++, a.load(), a.play(), $("#solution-video-h5").removeClass("vjs-seeking")
					}), this.on("ended", function() {
						a.load(), a.pause(), $("#solution-video-h5").removeClass("vjs-seeking")
					})
				});
				if ($("#about-video-h5").length > 0) var vjsSolution = videojs("#about-video-h5", {
					controls: !0,
					techOrder: ["html5", "flash"],
					autoplay: !1,
					preload: "none",
					...
					this.on("loadstart", function() {
						...
						b = a.currentTime(), c++, a.load(), a.play(), $("#about-video-h5").removeClass("vjs-seeking")
					}), this.on("ended", function() {
						a.load(), a.pause(), $("#about-video-h5").removeClass("vjs-seeking")
					})
				});
				$(".solution-video-toggle").click(function(a) {
					a.preventDefault(), $(".video-modal-wrapper").fadeIn(function() {
						vjsSolution.play(), vjsSolution.autoplay(!0)
					})
				}), $(".video-modal-close").click(function(a) {
					a.preventDefault(), vjsSolution.pause(), $(".video-modal-wrapper").fadeOut()
				})
			} else $("#solution-video-h5").remove(), $(".solution-video-toggle").click(function(a) {
				if (a.preventDefault(), FlashDetect.versionAtLeast(10)) {
					var b = $("#solution-video-flash").data("url"),
						c = {
							autostart: !0,
							file: b
						},
						d = {
							width: 800,
							height: 450,
							modId: "solution-video-flash",
							params: c
						};
					createFlashPlayer(d)
				} else $("#solution-video-flash").css({
					position: "relative",
					width: 800,
					height: 450,
				...
				a.preventDefault(), $(".video-modal-wrapper").fadeOut()
			});
		var data_switch_obj = {
			$switch: $(".J_anchor-switch"),
			eventMethod: "click",
			isWatchUrl: !0
		};
		$(".part-content.J_anchor-target").first().show(), anchorSwitch(data_switch_obj, function(a) {
			a.show().siblings(".J_anchor-target").hide()
		});
		var $supportBtn = $(".J_support-btn"),
			support_url_before = "http://b.qq.com/webc.htm?new=0&sid=4000188113&eid=218808P8z8p8x8Q808Q8Q&o=http://www.ucloud.cn&q=7&ref=http://www.ucloud.cn/help",
			support_url_after = "http://b.qq.com/webc.htm?new=0&sid=4000188113&eid=218808P8z8p8x8Q808Q8Q&o=http://www.ucloud.cn&q=7&ref=http://www.ucloud.cn/help";
		$(".J_close-modal").click(function() {
			$(".modal-laywer").fadeOut()
		}), $supportBtn.click(function(a) {
			a.preventDefault(), supportUrl($(this))
		});
		var $upn_tab = $(".J_upn-tab");
		equalDiv({
			parent: $upn_tab,
			child: "li"
		}), $(window).resize(function() {
		...
		anchorSwitch(upn_switch_obj, function(a) {
			a.show().siblings(".J_anchor-target").hide()
		}), equalDiv({
			parent: $(".upn-support-wrapper"),
			child: ".upn-support",
			width: 1200
		})
	});
var isProduction = !0;
$("body").scrollspy({
	target: "#navbar-example"// 控制eq()初始化0
}), Dropdown.prototype.init = function() {
	this.set(0), this.bind()
}, Dropdown.prototype.set = function(a) {
	null == a && (a = 0), this.selectIndex = a, this.selectDom.val(this.dropdownDoms.eq(a).text().trim()), this.linkageDoms && this.linkageDoms.hide().eq(a).show(), this.dropdownDomWrapper.hide(), this.callback && this.callback()
}, Dropdown.prototype._selectDomClickHandle = function() {
	this.dropdownDomWrapper.toggle()
}, Dropdown.prototype._dropdownDomsClickHandle = function(a) {
	var b = $(a.currentTarget).index();
	this.set(b)
}, Dropdown.prototype.bind = function() {
	var a = this;
	this.selectDom.parent().bind("click", function(b) {
		a._selectDomClickHandle.apply(a, arguments)
	}), this.dropdownDoms.bind("click", function(b) {
		a._dropdownDomsClickHandle.apply(a, arguments)
	})
}, Dropdown.prototype.unbind = function() {
	this.selectDom.parent().unbind("click"), this.dropdownDoms.unbind("click")
}, Dropdown.prototype.destory = function() {
	this.unbind(), this.selectDom = null, this.dropdownDomWrapper = null, this.dropdownDoms = null, this.linkageDoms = null
};